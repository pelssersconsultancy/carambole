<script lang="ts">
  ...
  function isInProgress(): boolean {
    return GameMode.InProgress === $gameStore.gameMode;
  }
  ...
  $: isEnteredScorePlayer1Invalid =
    parseInt($gameStore.player1.enteredScore, 10) >
    $gameStore.player1.carambolesLeft;
  $: isEnteredScorePlayer2Invalid =
    parseInt($gameStore.player2.enteredScore, 10) >
    $gameStore.player2.carambolesLeft;

  function handleKeyPressFinishedGame(event) {
    console.log('GameGrid handleKeyPressFinishedGame', event);
  ...
  function handleKeyPress(event) {
    if (isFinished()) {
      handleKeyPressFinishedGame(event);
    }

    if (isInProgress()) {
      handleKeyPressInProgressGame(event);
    }
  }
</script>
...
<section class="grid h-full grid-cols-3 text-gray-100 justify-items-center">
  ...
  <GridCell>
    <div
      ...
      class:bg-red-700={game.player1.player === game.currentPlayer &&
        GameMode.InProgress === game.gameMode}
    >
      {game.player1.name}
    </div>
  </GridCell>
  ...
  <GridCell
    ><div
      ...
      style="background-image: url('./images/bonanza.jpg')"
    /></GridCell
  >
  <GridCell>
    <div
      class="w-4/5 text-6xl font-bold grid-cell-content h-4/5"
      class:bg-red-700={game.player2.player === game.currentPlayer &&
        GameMode.InProgress === game.gameMode}
    >
      {game.player2.name}
    </div>
  ...
  <GridCell borderStyle="solid">
    <div class="text-6xl font-bold text-green-500 grid-cell-content">
      {game.player1.totalCaramboles} / {game.player1.carambolesToMake}
    </div>
  </GridCell>
  <GridCell borderStyle="solid">
    <div class="text-6xl font-bold text-green-500 grid-cell-content">
  ...
  <GridCell borderStyle="solid">
    <div class="text-6xl font-bold text-green-500 grid-cell-content">
      {game.player2.totalCaramboles} / {game.player2.carambolesToMake}
    </div>
  </GridCell>
  ...
  <GridCell borderStyle="solid">
    <div
      class="h-full font-bold leading-none text-yellow-500 grid-cell-content text-11xl"
      class:text-yellow-500={!isEnteredScorePlayer1Invalid}
      class:text-red-500={isEnteredScorePlayer1Invalid}
    >
      {game.player1.enteredScore}
    </div>
  </GridCell>
  <!-- @Emile: je kunt ook nog e.g. borderColor="red-300" borderSize="{8}" invullen -->
  <GridCell borderStyle="solid">
    <div
      class="flex-col h-full text-6xl font-bold leading-none text-blue-500 grid-cell-content"
    >
      {#if isInProgress() && game.lastTurn}<span class="text-red-600">gelijkmakende&nbsp;</span
        >{/if}
      {#if game.gameMode === GameMode.Finished}
        Spel afgelopen <img
          src="images/finish-line-flag.svg"
          class="w-14 h-14"
          alt="Einde spel"
        />
      {/if}
  ...
  <GridCell borderStyle="solid">
    <div
      class="h-full font-bold leading-none grid-cell-content text-11xl"
      class:text-yellow-500={!isEnteredScorePlayer2Invalid}
      class:text-red-500={isEnteredScorePlayer2Invalid}
    >
      {game.player2.enteredScore}
    </div>
  </GridCell>

  <!-- RIJ 4 gemiddelde van gemaakte caramboles-->
  <GridCell>
    <div class="text-6xl font-bold grid-cell-content">
      {game.player1.averageCaramboles.toFixed(3)}
    </div>
  </GridCell>
  <GridCell>
  ...
  <GridCell>
    <div class="text-6xl font-bold grid-cell-content">
      {game.player2.averageCaramboles.toFixed(3)}
    </div>
  </GridCell>

  <!-- RIJ 5 hoogste score -->
  <GridCell>
    <div class="text-5xl font-bold grid-cell-content">
      {game.player1.highestSerie}
    </div>
  </GridCell>
  <GridCell>
